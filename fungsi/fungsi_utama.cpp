...
void muat_data()
{
    ...
    if (!file.is_open()) {
        cout << MERAH << " [!] Gagal membuka file data_transaksi.csv. Memulai dengan data kosong. " << RESET
...
void simpan_data()
{
    ...
    if (!file.is_open()) {
        cout << MERAH << " [!] Gagal membuka file data_transaksi.csv untuk disimpan. " << RESET
             << "\n"; // // jika gagal membuka file CSV maka tampilkan pesan error
        return; // keluar dari fungsi
    }
    ...
    cout << BG_BIRU << PUTIH << " >> Data tersimpan otomatis. " << RESET << "\n"; // menampilkan pesan sukses penyimpanan data
}
...
void tambah_transaksi()
{
    ...
    if (jumlah_data >= MAX_DATA) { // memeriksa apakah jumlah data sudah mencapai batas maksimum
        cout << BG_MERAH << PUTIH << " [ERROR] Penyimpanan penuh! " << RESET << "\n"; // jika data sudah penuh maka tampilkan pesan error
    ...
    cout << "Pilih: " << HIJAU << "[1] Pemasukan" << RESET << " / " << MERAH << "[2] Pengeluaran" << RESET << "\n";
    ...
    do {
        ...
        if (!validasi_tanggal(baru.tanggal)) {
            cout << MERAH << "   Format salah! Harus 10 karakter.\n" << RESET;
    ...
    cout << "\n" << BG_HIJAU << HITAM << " [SUKSES] Data disimpan! " << RESET << "\n"; // menampilkan pesan sukses penambahan data
    enter_untuk_lanjut(); // menunggu input enter dari pengguna
}
...
void edit_transaksi(int indeks)
{
    ...
    cout << "\n" << BG_KUNING << HITAM << " --- Edit Data (Enter = Lewati) --- " << RESET << "\n"; // menampilkan header edit data
...
void kelola_riwayat()
{
    while (true) {
        ...
        for (int i = 0; i < jumlah_data; i++) { // looping untuk setiap data transaksi yang ada
            cetak_baris_tabel(i + 1, daftar_transaksi[i]); // mencetak satu baris data transaksi saat ini dengan format tabel
        }
...
void tampilkan_skala_prioritas()
{
    ...
    for (int i = 0; i < count; i++) { // looping untuk setiap data transaksi pengeluaran yang ada di array sementara
        cetak_baris_tabel(i + 1, temp[i]); // mencetak satu baris data transaksi pengeluaran saat ini dengan format tabel
    }
    cetak_garis_tabel(); // mencetak garis tabel (bawah)
    enter_untuk_lanjut(); // menunggu input enter dari pengguna
}
...
void atur_budget()
{
    ...
    if (budget_limit_baru < 0) {
        cout << MERAH << "Tidak boleh negatif!" << RESET << "\n";
...
void cek_status_budget()
{
    ...
    cout << " Terpakai       : " << ((pengeluaran > budget_limit) ? MERAH : HIJAU) << "Rp " << pengeluaran << RESET
    ...
    if (sisa < 0) { cout << "\n" << BG_MERAH << PUTIH << " [!!!] MELEBIHI BATAS ANGGARAN! " << RESET << "\n"; }
